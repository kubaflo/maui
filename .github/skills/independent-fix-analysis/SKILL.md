...
gh pr view XXXXX --json body | jq -r '.body' | grep -oE "#[0-9]+" | head -1
...
## Example: CollectionView Template ID Fix

**Issue**: Item templates not rendering correctly with `MeasureFirstItem=true`

**Root Cause**: Code checks `template.Id == DefaultItemTemplateId` but DataTemplateSelector generates dynamic IDs

**Alternative Fixes**:
1. Check if ID is default OR created by DataTemplateSelector (complex)
2. Never skip optimization for DataTemplateSelector (simpler)

**Potential Regression**: Fix uses `template.Id == DefaultItemTemplateId` which won't match DataTemplateSelector's dynamic IDs
