...
Verifies that tests actually catch the issue. Supports **all test types** (UI tests, device tests, unit tests, XAML tests) and two workflow modes.
...
# Unit tests (no platform needed)
pwsh .github/skills/verify-tests-fail-without-fix/scripts/verify-tests-fail.ps1 -RequireFullVerification
...
2. Auto-detects **all test types** from changed files (UI, device, unit, XAML)
3. Auto-detects test classes/filters from changed test files
4. Runs each detected test using the appropriate runner
5. **Updates PR labels** based on result
6. Reports result
...
3. Auto-detects all tests and their types from changed files
4. Reverts fix files to base branch
5. Runs all detected tests (should FAIL without fix)
6. Restores fix files
7. Runs all detected tests (should PASS with fix)
