...
foreach ($issue in $issues) {
    ...
    # 2. Reproduction check (heuristic: look for code blocks, numbered steps, XAML)
    ...
    $hasRepro = $hasCodeBlock -or $hasNumberedSteps -or $hasXaml
    $hasMinimalInfo = $bodyLength -gt 200

    if (-not $hasRepro -and -not $hasMinimalInfo) {
        $flags += "needs-repro"
        $suggestions += "Issue body is short ($bodyLength chars) with no code blocks or numbered steps."
    }
    elseif (-not $hasRepro -and $hasMinimalInfo) {
        $flags += "weak-repro"
        $suggestions += "Issue has description but no code blocks, XAML, or numbered steps."
    }
