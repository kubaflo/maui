...
**Critical PR hygiene issues must be fixed before approval.**
...
**⚠️ CRITICAL PR HYGIENE ISSUE**: This PR contains changes for TWO separate issues (#19806 AND #33420). This violates the one-issue-per-PR principle and must be fixed.
...
|-----------|---------------|-------------|--------|
| LayoutExtensions.cs:48 | Avoid computing IsExplicitSet twice (efficiency) | Not addressed in code | ⚠️ NOT FIXED |
| LayoutExtensions.cs:91 | Create variable at method start | Not addressed in code | ⚠️ NOT FIXED |
| LayoutExtensions.cs:48 | Fix vertical constraint too (same bug) | Not addressed in PR | ⚠️ NOT FIXED - SCOPE CREEP |
...
|---|--------|----------|-------------|---------------|-------|
| PR | PR #20133 | Add logic in LayoutExtensions to respect DesiredSize when width not explicitly set and HorizontalAlignment != Fill | ⏳ PENDING (Gate) | `LayoutExtensions.cs` (+10) | Original PR fix |

**PR's Approach (from code inspection):**
In `LayoutExtensions.cs`, when computing consumed width:
- If width not explicitly set AND HorizontalAlignment != Fill
- Use `Math.Min(bounds.Width, view.DesiredSize.Width)` instead of ignoring DesiredSize
- This allows button to measure at its desired (truncated) width

**Note:** try-fix candidates will be added during Phase 4 after Gate passes.
